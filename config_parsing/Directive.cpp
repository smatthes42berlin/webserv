...
/*   Updated: 2024/11/08 17:48:52 by smatthes         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
std::string Directive::get_directive_name(void)
{
	...
	if (this->_allowed_values.size() > 0)
	{
				...
				+ 1; it != this->_key_val.end(); ++it)
		{
			if (std::find(this->_allowed_values.begin(),
				...
				throw UnallowedArgumentForDirective();
		}
	}
}
...
const char *Directive::DuplicateIdentifier::what() const throw()
{
	...
	msg += this->_directive_name.c_str();
	msg += "\n";
